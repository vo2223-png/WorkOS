...
(() => {
  ...
  // Duplicate-card flows need to carry a slot's dropped image to a new id
  // without reaching into this module's private store.
  window.ImageSlot = window.ImageSlot || {};
  window.ImageSlot.cloneSlot = function (fromId, toId) {
    const v = getSlot(fromId);
    if (v) setSlot(toId, Object.assign({}, v));
  };
})();
